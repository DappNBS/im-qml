...
void RegisterConnection::sendGreetingMessage(){
    ...
    QByteArray msg = "GREETING " + QByteArray::number(greet.size())
            + ' ' + greet;
...
bool RegisterConnection::sendPeriodicRegisterMessage(){
    ...
    QByteArray msg = "REGISTER " + QByteArray::number(regist.size())
            + ' ' + regist;
...
int RegisterConnection::dataLengthForCurrentDataType(){
    if(this->bytesAvailable() <= 0 || this->readDataIntoBuffer() <= 0
            || !this->buffer.endsWith(SeparateToken))
        return 0;
...
bool RegisterConnection::hasEnoughData(){
    ...
    if(bytesAvailable() < this->numBytesForCurrentDataType
